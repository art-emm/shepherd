...
function parseAttachTo(step) {
  ...
  if (Array.isArray(options.element)) {
    var elements = [];
    options.element.forEach(function (element) {
...
function getTetherOptions(attachToOptions, step) {
  ...
  if (!attachToOptions.element || Array.isArray(attachToOptions.element) && attachToOptions.element.length > 1 || !attachToOptions.on) {
...
function init(component, options, instance, create_fragment, not_equal, props) {
  ...
  var ready = false;
  $$.ctx = instance ? instance(component, prop_values, function (key, ret, value) {
    if (value === void 0) {
      value = ret;
    }
  ...
  return {
    c: function c() {
      ...
      dispose = listen(button, "click", function () {
        ctx.action.apply(this, arguments);
      });
    },
    ...
    p: function p(changed, new_ctx) {
      ctx = new_ctx;
      if (changed.text) set_data(t, ctx.text);
...
function instance($$self, $$props, $$invalidate) {
  ...
  $$self.$$.update = function (changed) {
    ...
    if (changed.config || changed.step) {
       {
...
function create_fragment$6(ctx) {
  var div;
  var t0;
  var show_if_1 = !isUndefined$1(ctx.step.options.text);
  var t1;
  var show_if = Array.isArray(ctx.step.options.buttons) && ctx.step.options.buttons.length;
  var current;
...
function (_Evented) {
  ...
  _proto._show = function _show() {
    ...
    if (Array.isArray(target)) {
      target.forEach(function (el) {
        el.classList.add(_this4.classPrefix + "shepherd-enabled", _this4.classPrefix + "shepherd-target");
      });
    } else {
  ...
  _proto._updateStepTargetOnHide = function _updateStepTargetOnHide() {
    ...
    if (this.options.highlightClass) {
      if (Array.isArray(this.target)) {
        this.target.forEach(function (el) {
          el.classList.remove(_this5.options.highlightClass);
        });
      } else {
    ...
    if (Array.isArray(this.target)) {
      this.target.forEach(function (el) {
        el.classList.remove(_this5.classPrefix + "shepherd-enabled", _this5.classPrefix + "shepherd-target");
      });
    } else {
...
function create_fragment$8(ctx) {
  ...
  return {
    c: function c() {
      ...
      attr(path, "d", path_d_value = "" + (ctx.svgPath || ctx.defaultOverlay));
    ...
    p: function p(changed, ctx) {
      if ((changed.svgPath || changed.defaultOverlay) && path_d_value !== (path_d_value = "" + (ctx.svgPath || ctx.defaultOverlay))) {
...
function instance$8($$self, $$props, $$invalidate) {
  var element = $$props.element,
      ...
      _$$props$svgPath = $$props.svgPath,
      svgPath = _$$props$svgPath === void 0 ? "" : _$$props$svgPath,
      _$$props$defaultOverl = $$props.defaultOverlay,
      defaultOverlay = _$$props$defaultOverl === void 0 ? "M 0 0 H " + window.innerWidth + " V " + window.innerHeight + " H 0 L 0 0 Z" : _$$props$defaultOverl;
  var guid = uuid();
  ...
  function positionModalOpening(targetElement, scrollParent, modalOverlayOpeningPadding) {
    ...
    $$invalidate("svgPath", svgPath = "");
    targetElement.forEach(function (element) {
      if (element.getBoundingClientRect) {
        var _getVisibleHeight2 = _getVisibleHeight(element, scrollParent),
        ...
        x = (x || left) - modalOverlayOpeningPadding, y = y - modalOverlayOpeningPadding, width = width + modalOverlayOpeningPadding * 2, height = height + modalOverlayOpeningPadding * 2;
        $$invalidate("svgPath", svgPath += "M " + x + " " + y + " H " + (width + x) + " V " + (height + y) + " H " + x + " L " + x + " 0 Z ");
      }
    });
    $$invalidate("svgPath", svgPath += defaultOverlay);
  }
  ...
  $$self.$set = function ($$props) {
    ...
    if ("svgPath" in $$props) $$invalidate("svgPath", svgPath = $$props.svgPath);
    if ("defaultOverlay" in $$props) $$invalidate("defaultOverlay", defaultOverlay = $$props.defaultOverlay);
  };
